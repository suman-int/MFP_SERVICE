...
//                List<ContactReportDealerPersonnel> contactReportDealerPersonnels = contactReportDealerPersonnelRepository.saveAll(newList);
	...
	public String submitReportData(ContactReportInfoDto report, MFPUser mfpUser) throws Exception {
		...
		try {
			...
			reportInfo.setContactType(
					report.getContactType() != null ? report.getContactType() : reportInfo.getContactType());
			String reps = report.getCorporateReps();
			reportInfo.setCorporateReps(reps != null ? (reps) : reportInfo.getContactType());
			if (!CollectionUtils.isEmpty(report.getDiscussions())) {
				reportInfo.setDiscussions(report.getDiscussions());
				reportInfo.setCurrentIssues(report.getDiscussions().stream().map(ContactReportDiscussion::getTopic)
						.filter(Objects::nonNull).collect(Collectors.joining("|")));
			 ...
			 * if (!CollectionUtils.isEmpty(report.getDealerPersonnels()) &&
			 * !CollectionUtils.isEmpty(reportInfo.getDealerPersonnels())) {
			 * List<ContactReportDealerPersonnel> removedList = new ArrayList<>(); for(int i
			 * = 0 ; i < reportInfo.getDealerPersonnels().size(); i++) {
			 * ContactReportDealerPersonnel existingDp =
			 * reportInfo.getDealerPersonnels().get(i); boolean found = false; for(int j = 0
			 * ; j < report.getDealerPersonnels().size(); j++ ) {
			 * ContactReportDealerPersonnel newDp = report.getDealerPersonnels().get(j); if(
			 * newDp.getPersonnelIdCd().equalsIgnoreCase(existingDp.getPersonnelIdCd())) {
			 * found = true; break; } } if( ! found ) { existingDp.setPersonnelId(i);
			 * removedList.add(existingDp); } } if (!CollectionUtils.isEmpty(removedList)) {
			 * contactReportDealerPersonnelRepository.saveAll(removedList); } } else
			 */
			if (!CollectionUtils.isEmpty(report.getDealerPersonnels())) {
	...
	public ContactReportDto findByContactReportId(long contactReportId) {
		...
		crInfo.setDealerPersonnels(crInfo.getDealerPersonnels().stream()
