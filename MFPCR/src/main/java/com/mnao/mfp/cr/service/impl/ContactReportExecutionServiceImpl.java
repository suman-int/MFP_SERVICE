...
public class ContactReportExecutionServiceImpl implements ContactReportExecutionService {
	...
	public List<ContactReportExecutionCoverageDto> reportExecutionCoverageByReportTime(FilterCriteria filterCriteria,
		...
		List<ContactReportInfo> contactReportInfoList = contactInfoRepository
				.findByContactDtBetweenAndContactStatusGreaterThanAndIsActive(filterCriteria.getStartDate(),
						filterCriteria.getEndDate(), ContactReportEnum.DRAFT.getStatusCode(),
		...
		Map<Dealers, Map<String, List<ContactReportInfo>>> reportMap = contactReportInfoList.stream().collect(Collectors
		...
		reportMap.forEach((key, value) -> {
			...
			value.forEach((key1, value1) -> {
				...
				boolean isServiceRetentionFysl = value1.stream().filter(cr -> Objects.nonNull(cr.getCurrentIssues()))
