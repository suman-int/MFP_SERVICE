...
//                            contactReportDiscussion.getDiscussion().equals(issueType)
    ...
    public List<SummaryByDealerListDto> summaryByCurrentStatusDealershipList(String issue) {
        ...
        return dealerList.stream().map(dlr -> SummaryByDealerListDto.builder().cityName(dlr.getCityNm()).dealerCode(dlr.getDlrCd())
                .dealerName(dlr.getDbaNm()).issue(issue).zipCode(dlr.getZipCd()).stateName(dlr.getStCd()).build()).collect(Collectors.toList());
    ...
    private Map<String, Map<String, List<ContactReportInfo>>> filterContactReportsByLocation(FilterCriteria filter, List<ContactReportInfo> contactReports) {
        Map<String, Map<String, List<ContactReportInfo>>> reports;
    ...
    private Stream<ContactReportInfo> filterContactReportByRegion(List<ContactReportInfo> contactReports, String filter) {
        return contactReports.stream()
                .filter(cr -> filter.equalsIgnoreCase(cr.getDealers().getRgnCd()));
    }

    private Stream<ContactReportInfo> filterContactReportByZone(List<ContactReportInfo> contactReports, FilterCriteria filter) {
    ...
    private Stream<ContactReportInfo> filterContactReportByDistrict(List<ContactReportInfo> contactReports, FilterCriteria filter) {
    ...
    private List<ContactReportInfo> filterContactReportsByDateRange(FilterCriteria filter, List<ContactReportInfo> contactReports) {
        return contactReports.stream()
                .filter(cr -> cr.getContactDt().isAfter(filter.getStartDate()) && cr.getContactDt().isBefore(filter.getEndDate()))
                .collect(Collectors.toList());
    }

    private List<ContactReportInfo> filterContactReportsByIssues(FilterCriteria filter, List<ContactReportInfo> contactReports) {
        return contactReports.stream()
                .filter(cr -> filter.getIssuesFilter().stream()
                        .anyMatch(value -> cr.getCurrentIssues().contains(value)))
                .collect(Collectors.toList());
    }
