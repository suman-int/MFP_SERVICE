SELECT 	PRSN_ID_CD, STATUS_CD, PRSN_TYPE_CD, FRST_NM, MIDL_NM, LAST_NM, JOB_CD, LOCTN_CD, USERID, EMAIL_ADDR, 
		RGN_CD, ZONE_CD, DISTRICT_CD	
FROM 	(
		SELECT 	DISTINCT A.PRSN_ID_CD, A.STATUS_CD, A.PRSN_TYPE_CD, A.FRST_NM, A.MIDL_NM, A.LAST_NM, B.JOB_CD, B.LOCTN_CD, C.USERID, C.EMAIL_ADDR, 
		SUBSTR(B.LOCTN_CD, 1, 2) RGN_CD, ' ' ZONE_CD, SUBSTR(B.LOCTN_CD, 4, 2) DISTRICT_CD    
		FROM 	$SCHEMA$BTC03020 A,
				$SCHEMA$BTC03050 B,
				$SCHEMA$BTC03110 C 
		WHERE   A.PRSN_ID_CD = B.PRSN_ID_CD 
		AND 	A.PRSN_ID_CD = C.PRSN_ID_CD 
		AND		PRSN_TYPE_CD = 'M'
		AND 	C.END_DT = '0001-01-01'
		AND 	B.JOB_CD IN ('MB11', 'MC11', 'MD11', 'ME11', 'MO11', 'MP11')
		UNION
		SELECT 	DISTINCT A.PRSN_ID_CD, A.STATUS_CD, A.PRSN_TYPE_CD, A.FRST_NM, A.MIDL_NM, A.LAST_NM, B.JOB_CD, B.LOCTN_CD, C.USERID, C.EMAIL_ADDR, 
		SUBSTR(B.LOCTN_CD, 1, 2) RGN_CD, SUBSTR(B.LOCTN_CD, 4, 2) ZONE_CD, ' ' DISTRICT_CD    
		FROM 	$SCHEMA$BTC03020 A,
				$SCHEMA$BTC03050 B,
				$SCHEMA$BTC03110 C 
		WHERE   A.PRSN_ID_CD = B.PRSN_ID_CD 
		AND 	A.PRSN_ID_CD = C.PRSN_ID_CD 
		AND		PRSN_TYPE_CD = 'M'
		AND 	C.END_DT = '0001-01-01'
		AND 	B.JOB_CD = 'MZ11'
		ORDER BY JOB_CD 
		) X
WHERE X.RGN_CD = ?
