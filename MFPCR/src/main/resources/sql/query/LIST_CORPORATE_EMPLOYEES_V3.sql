...
FROM 	(
		...
		AND		PRSN_TYPE_CD = 'M'
		AND 	C.END_DT = '0001-01-01'
		AND 	B.JOB_CD IN ('MG11', 'MO11', 'MF11')
		AND 	SUBSTR(B.LOCTN_CD, 1, 2) = ?
		UNION
		SELECT 	DISTINCT A.PRSN_ID_CD, A.STATUS_CD, A.PRSN_TYPE_CD, A.FRST_NM, A.MIDL_NM, A.LAST_NM, B.JOB_CD, B.LOCTN_CD, C.USERID, C.EMAIL_ADDR, 
		...
		FROM 	$SCHEMA$BTC03020 A
		WHERE   PRSN_TYPE_CD = 'M'
		AND 	B.LOCTN_CD = 'MA92'
		AND 	(CASE WHEN A.TRMNTN_DT = DATE('0001-01-01') THEN DATE('9999-12-31') ELSE A.TRMNTN_DT END) BETWEEN ? AND CURRENT DATE
		) X
ORDER BY LAST_NM, FRST_NM, MIDL_NM